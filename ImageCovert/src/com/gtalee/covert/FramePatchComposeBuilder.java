...
public final class FramePatchComposeBuilder {
  ...
  private static final int SAFE_SINGLE_PATCH_PIXELS = 60000;
  ...
  private static final class Piece { BufferedImage image; BufferedImage trimmed; Boolean singleValid; int x,y,frame; String name; Piece(BufferedImage i,int px,int py,int f){image=i;x=px;y=py;frame=f;} BufferedImage trimmed(){if(trimmed==null)trimmed=trimPip(image);return trimmed;} boolean singleValid()throws IOException{if(singleValid==null){BufferedImage t=trimmed();if(t.getWidth()*t.getHeight()<=SAFE_SINGLE_PATCH_PIXELS)singleValid=Boolean.TRUE;else{List<BufferedImage> one=new ArrayList<BufferedImage>();one.add(t);singleValid=Boolean.valueOf(WorkshopPimValidator.validate(one).valid);}}return singleValid.booleanValue();} }
  ...
  public static Result build(File input,File output,int threshold,int gap,int minPixels,int maxPatch,int paletteColors,int alphaThreshold,boolean dither,ProgressListener listener)throws IOException{
    ...
    int groups=buildGroups(p,pieces,baseMapped,palette,groupByName,listener);p.setProperty("pim.groups",String.valueOf(groups));r.pipGroupCount=groups;
  ...
  /* Find the largest valid prefix using exponential probing and binary search. */
  private static int buildGroups(Properties p,List<Piece> pieces,BufferedImage base,int[] palette,Map<String,Integer> groupByName,ProgressListener listener)throws IOException{
    ...
    Map<String,WorkshopPimValidator.Result> checks=new HashMap<String,WorkshopPimValidator.Result>();
    while(start<pieces.size()){
      int remaining=pieces.size()-start,low=0,high=Math.min(SAFE_PIM_GROUP_SIZE,remaining);
      while(validateRangeCached(groups==0?baseParts:new ArrayList<BufferedImage>(),pieces,start,high,checks,palette)){low=high;if(low>=remaining)break;high=Math.min(remaining,high<<=1);progress(listener,"PIM probe: "+(start+low)+"/"+pieces.size(),start+low,pieces.size());}
      if(low==0)throw new IOException("A single patch cannot fit in PIM group");
      if(high>remaining)high=remaining;
      while(low+1<high){int mid=low+(high-low)/2;if(validateRangeCached(groups==0?baseParts:new ArrayList<BufferedImage>(),pieces,start,mid,checks,palette))low=mid;else high=mid;progress(listener,"PIM boundary: "+(start+mid)+"/"+pieces.size(),start+mid,pieces.size());}
      List<BufferedImage> groupBase=groups==0?baseParts:new ArrayList<BufferedImage>();
      WorkshopPimValidator.Result groupCheck=validateRangeExact(groupBase,pieces,start,low,palette);
      if(!groupCheck.valid){int exactLow=0,exactHigh=low;while(exactLow+1<exactHigh){int mid=exactLow+(exactHigh-exactLow)/2;WorkshopPimValidator.Result c=validateRangeExact(groupBase,pieces,start,mid,palette);if(c.valid)exactLow=mid;else exactHigh=mid;}if(exactLow==0)throw new IOException("A single patch cannot fit in PIM group: "+groupCheck.reason);low=exactLow;groupCheck=validateRangeExact(groupBase,pieces,start,low,palette);}
      List<BufferedImage> images=new ArrayList<BufferedImage>();images.addAll(groupBase);List<Piece> groupPieces=new ArrayList<Piece>();
      for(int i=0;i<low;i++){Piece piece=pieces.get(start+i);images.add(piece.trimmed());groupPieces.add(piece);}
      writeGroup(p,groups,groupPieces,groupCheck);for(Piece piece:groupPieces)groupByName.put(piece.name,Integer.valueOf(groups));
      groups++;start+=low;progress(listener,"PIM group "+groups+": "+start+"/"+pieces.size(),start,pieces.size());
    }
    return groups;
  }  private static WorkshopPimValidator.Result validateRangeExact(List<BufferedImage> base,List<Piece> pieces,int start,int count,int[] palette)throws IOException{List<BufferedImage> images=new ArrayList<BufferedImage>(base.size()+count);images.addAll(base);for(int i=0;i<count;i++)images.add(pieces.get(start+i).trimmed());return WorkshopPimValidator.validateWithPalette(images,palette);}
  private static boolean validateRangeCached(List<BufferedImage> base,List<Piece> pieces,int start,int count,Map<String,WorkshopPimValidator.Result> checks,int[] palette)throws IOException{String key=start+":"+count;WorkshopPimValidator.Result cached=checks.get(key);if(cached!=null)return cached.valid;List<BufferedImage> images=new ArrayList<BufferedImage>(base.size()+count);images.addAll(base);for(int i=0;i<count;i++)images.add(pieces.get(start+i).trimmed());WorkshopPimValidator.Result result=WorkshopPimValidator.validateProbeWithPalette(images,palette);checks.put(key,result);return result.valid;}
  private static boolean validateRange(List<BufferedImage> base,List<Piece> pieces,int start,int count)throws IOException{
    ...
    for(int i=0;i<count;i++)images.add(pieces.get(start+i).trimmed());
    return WorkshopPimValidator.validate(images).valid;
  }
  private static void splitRecursive(BufferedImage im,int x,int y,int frame,List<Piece> out)throws IOException{BufferedImage trimmed=trimPip(im);if(im.getWidth()<=255&&im.getHeight()<=255){Piece candidate=new Piece(im,x,y,frame);candidate.trimmed=trimmed;if(trimmed.getWidth()*trimmed.getHeight()<=SAFE_SINGLE_PATCH_PIXELS)candidate.singleValid=Boolean.TRUE;else{List<BufferedImage> one=new ArrayList<BufferedImage>();one.add(trimmed);candidate.singleValid=Boolean.valueOf(WorkshopPimValidator.validate(one).valid);}if(candidate.singleValid.booleanValue()){out.add(candidate);return;}}if(im.getWidth()<=1&&im.getHeight()<=1)throw new IOException("PIM容量预检无法切分图块");if(im.getWidth()>=im.getHeight()){int a=im.getWidth()/2;splitRecursive(im.getSubimage(0,0,a,im.getHeight()),x,y,frame,out);splitRecursive(im.getSubimage(a,0,im.getWidth()-a,im.getHeight()),x+a,y,frame,out);}else{int a=im.getHeight()/2;splitRecursive(im.getSubimage(0,0,im.getWidth(),a),x,y,frame,out);splitRecursive(im.getSubimage(0,a,im.getWidth(),im.getHeight()-a),x,y+a,frame,out);}}
  ...
  private static void writeGroup(Properties p,int n,List<Piece> pieces,WorkshopPimValidator.Result c)throws IOException{if(!c.valid)throw new IOException("PIM容量预检失败，分组"+n+"："+c.reason);p.setProperty("pim.group."+n+".count",String.valueOf(pieces.size()));p.setProperty("pim.group."+n+".pdata",String.valueOf(c.pdataLength));p.setProperty("pim.group."+n+".maxIdata",String.valueOf(c.maxIdataLength));}
